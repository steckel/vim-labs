...
```vim
:RevueLocal HEAD       " Capture saved Git changes against HEAD
:RevueLocal! HEAD      " Also include untracked files (excluding ignored files)
:RevueLocalReviews     " List saved local reviews; Enter reopens one
```

Use `c` to comment, `r` to reply, and `C` for the conversation. In the composer,
`:w` retains a draft; Ctrl-S or `:RevueSend` saves it into the local review after
confirmation. Comments and replies survive restarting Vim. The displayed files
are frozen at capture time; `R` refreshes discussion. Run `:RevueLocal` again
...
`:RevueRunParticipant` previews starting or resuming a configured participant
from assignment outcomes. `:RevueAbandonRun` releases an unstarted preparation
through the same preview and receipt-recovery flow. See [runtime setup and validation limits](doc/participant-runtime.md).
Vim message selection, assignment preview and durable creation now work through `:RevueAssign`.
`:RevueAssignments` reads per-comment outcomes and opens their discussions/code;
`:RevueAssignmentDetails` exposes full identifiers and source references while
the outcome reader keeps metadata compact.
`:RevueCancelAssignment` previews revoking MCP access with durable recovery.
...
`:Revue` opens review mode against `g:revue_default_base` (default
`'main'`); `:Revue <revision>` diffs against a specific revision
...
actions and actual bindings for the current view. `:RevueThread` (or
...
body. `:RevueCopyLink a` copies its permalink into register `a` when available.
...
Use `:RevueReviewActions` (or `<LocalLeader>a`) for a contextual chooser grouped
...
`:RevueBodyLinks` lists labeled message links and their full destinations before
opening or copying. GitHub resolves them from its rendered comment HTML; other
backends can supply explicit relative-link mappings. `:RevueBodyURLs` retains
the raw URL finder, including code examples. Neither has a default key.
`:RevueQuoteAttributed` appends a quote with an escaped author and source permalink
...
When the backend permits it, `:RevueEditMessage` edits the selected published
message. Preview compares the original, current and proposed text. Conflicts
retain your draft; `:RevueEditBase` accepts a refreshed base without replacing
...
`:RevueMessageHistory` opens the selected message's available edits in a read-only
...
`:RevueDeleteMessage` previews deletion of one saved message when its backend
supports that scope. Close the preview, then use `:RevueSend` to confirm; local
...
`:RevueReactions` loads the selected message's choices; Enter executes the labeled
...
`:RevuePending` resumes a GitHub review saved privately in the browser. Open its
exact comments with Enter, or use `:RevuePublishPending` to prepare publication.
Preview includes every server comment and excludes local outbox drafts. If the
review changed, `:RevuePendingBase` accepts refreshed contents while retaining
your summary. `:RevueDeletePendingComment` previews deletion of one selected
private comment, from the pending inventory or focused discussion. Close the
preview and use `:RevueSend` to confirm. GitHub roots with replies remain
unavailable until their deletion scope is verified. `:RevueDiscardPending` previews the entire private review for
explicit deletion; unrelated local drafts are retained. Uncertain deletion checks
server state without repeating the write. `:RevueEditPending` edits the selected
private summary/comment with the same conflict preview as published edits;
`:RevueEditMessage` also works in a private discussion. Unfinished private edits
are surfaced before publication. `:RevueStartPending` prepares a private review;
`:RevueSavePending` prepares private delivery of an inline/suggestion draft,
...
Use `:RevueReplyPending` to keep a reply to a published thread private, or
`:RevueSavePending` to convert an editable reply draft. Existing text and unknown
outcomes are retained. Use `:RevueStageBatch` to select several local drafts for
private delivery. It can create a private review or add to the existing one.
Space selects items, Enter edits, and `:RevueSendBatch` confirms the exact set.
Inline comments, suggestions, file feedback and replies save one at a time.
The queue retains each receipt and stops on failure. `:RevueCheckReceipt` only
checks an uncertain step; continuing the remaining saves needs confirmation.
`:RevueUnpackBatch` returns only unsaved items when all outcomes are known.
...
`:RevuePreview` to inspect rendered Markdown and `:RevueClose` to return to the
...
These settings affect rich sessions; the original `:Revue` keymap is unchanged.
...
Use `:RevueFocus` (or `<LocalLeader>z`) to maximize the current code, discussion,
composer, or preview pane. `:RevueRestoreLayout` restores split sizes, or returns
...
closing them returns to the prior view and sizing. `:RevueFiles` hides or reopens
the file sidebar. In a focused source pane, `:RevueClose` restores the layout
...
Use `:RevueBatch` to collect saved drafts into one review. Space checks or
unchecks an item; Enter reopens it for editing. The queue shows the selected
count, review decision, comparison, and full message text. `:RevueSendBatch`
...
`:RevueUnpackBatch`; an uncertain batch stays frozen until receipt recovery,
including after restarting Vim. Native GitHub pending reviews use the separate private-save workflow described
above; `:RevueStageBatch` selects private delivery with per-item outcomes.

Select a suggestion message and use `:RevueApplySuggestion` to preview applying
...
block application. `:RevueLatest` opens the result; interrupted writes use the
original receipt. [Application workflow and limits](doc/suggestion-application.md).

Use `:RevueResolve` or `:RevueReopen` on a thread, or choose the action from its
...
Unknown state remains explicit. `:RevueCheckThreadState` checks an uncertain resolution;
...
`:RevueActivity` shows durable delivery outcomes, receipts, and refresh errors.
Open a pending operation with Enter; use `:RevueCopyReceipt [register]` to copy
...
`:RevueNextUnread` visits them; `:RevueMarkRead` acknowledges one message and
`:RevueMarkThreadRead` acknowledges a selected thread. Reading and refresh do
...
messages. `:RevueLoadMoreFeedback` loads the next page without changing the
selected result or unsaved text; `:RevueCancelFeedback` ignores an outstanding
...
`:RevueRefresh` keeps the current inventory until fresh pages cover its previously
loaded message IDs, or the fresh inventory is exhausted. When more is needed,
`:RevueContinueRefresh` reads one page; `:RevueCancelRefresh` retains the current
...
`:RevuePending` shows loaded private comments and their known total. Individual
comments retain their private identity and permissions inside code threads.
`:RevueVerifyPending` loads and verifies the complete selected review before
...
`:RevueDiscussions [text]` searches the loaded review's threads, replies,
...
`:RevueDiscussionFilter state unresolved`, `author morgan`, `anchor outdated`,
...
resolution coverage are separate from readable comments. `:RevueRefresh` retries
the review; failed reads preserve loaded feedback, selected results and drafts.

`:RevueFileFilter path src/` narrows the file list; status, threads and viewed
...
`:RevueViewed` and `:RevueUnviewed` acknowledge the selected tree file or current
...
`:RevueVerifyViewed` retries verification. Closing before a pending mark finishes
...
`:RevueComparisons` lists saved references and loads backend history when
supported; Enter opens a comparison. `:RevueLatest` opens the latest and `:RevuePreviousComparison` returns.
...
known newer comparison. `:RevueResumeComparison` retrieves the last saved
comparison after restarting Vim; `:RevueDraftComparison` inspects an open draft's
original comparison. `:RevueLoadHistory` reloads the inventory, and
`:RevueCopyComparison [register]` copies its immutable reference.
...
To compare versions, select a row and use `:RevueRangeStart [base|head]`, then
select another row and use `:RevueRangeEnd [base|head]` (both default to head).
Inspect the endpoints and backend semantics above the list, then
`:RevueOpenRange`. `:RevueClearRange` clears the selection. The action guide also
...
use `:RevueReanchorDraft`. Choose a file or source range in Latest, then use
`:[range]RevueReanchorHere`. The read-only preview shows old and new locations,
available code, and the unchanged draft text. `:RevueAcceptReanchor` saves the
move locally; `:RevueCancelReanchor` returns to the original draft. Closing the
...
From a discussion, `:RevueThreadComparison` opens its original code. Local
...
`:RevueReturnContext` returns to the exact message and position before the jump,
even if its managed panel was closed. After restarting Vim, saved original
context can reopen with `:RevueResumeComparison`; the earlier return position
is session-only, so use `:RevueLatest` for current code. Reading history never
moves a comment or draft to newer lines.

`:RevueReadiness` opens a read-only view of backend checks and review
...
`:RevueTimeline` opens typed backend history with authors, timestamps and event
links. For an unloaded discussion, `:RevueLoadEventDiscussion` fetches that
...
supports `:RevueCancelFeedback`, and leaves focus alone if you move elsewhere.
Repeat explicitly for another page; `:RevueLoadMoreFeedback` remains available
...
`:RevueOlderEvents` loads earlier history pages; `:RevueReloadTimeline` checks for
new activity. `:RevueCancelTimeline` stops accepting the current read while
...
is not copied into the outbox. `:RevueEventComparison` opens a verified retained
source comparison, and `:RevueReturnContext` returns to the same history event.
...
For local reviews, `:RevueCapture` prepares another capture of saved workspace
files against the same fixed base. Inspect its settings and use `:RevueSend`
to save it. `:RevueCapture all` (or `!`) includes untracked files;
`:RevueCapture tracked` excludes them. With no argument, it retains the last
capture's setting. `:RevueLatest` opens the result when you are ready.
Discussion, earlier source, and old drafts survive. No code is staged or committed,
and no thread is resolved automatically. From a discussion, `:RevueThreadComparison`
...
Use `:RevueSuggest` on a head-side diff line, or `:'<,'>RevueSuggest` on a
...
Use `:RevueFileComment` from the file list or a source pane for feedback about
...
`:RevueFileThreads` opens that file's discussions, with file comments first;
